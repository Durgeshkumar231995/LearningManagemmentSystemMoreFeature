...
public class CoursesController {
	...
	public ResponseEntity<List<CourseDto>> findPaginated(@PathVariable Integer pageNo,@PathVariable Integer pageSize) {
		
		List<Course> findPaginated = userService.findPaginated(pageNo, pageSize);
		
		List<CourseDto> allCourse = findPaginated.stream().map(course->modelMapper.map(course,CourseDto.class)).collect(Collectors.toList());
